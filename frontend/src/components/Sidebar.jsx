    ...
    {categories.map((category) => (
      <button
        ...
        onClick={() => {
          ...
          return navigate("/videoType/" + (category.type_id))
        }}
        style={{
          background: category.type_name === selectedCategory && "#FC1503",
          color: "white",
        }}
        key={category.type_id}
      >
        <span style={{ color: category.type_name === selectedCategory ? "white" : "red", marginRight: "15px" }}>
          <i className={`${category.icon}`}></i>
        </span>
        <span style={{ opacity: category.type_name === selectedCategory ? "1" : "0.8" }}>
          {category.type_name}
        </span>
      </button>
    ))}
